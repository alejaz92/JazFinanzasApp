...
**Dos modalidades, una sola puerta.** Un `CardTransactionDiscount` tiene `CreditTarget`: `ACCOUNT` (el banco acreditó en una cuenta) o `CARD` (quedó como saldo a favor de la tarjeta). La diferencia es *cuándo* la plata entra a una cuenta, no cómo se consume después.

Todo pasa por `CardTransactionDiscountService.MaterializeAsync` — la única función que genera `Transaction` a partir de un descuento. La usan los tres caminos: el alta en modalidad cuenta (materializa el 100% en el acto), el resumen que absorbe saldo a favor (`CardTransactionService.ApplyCardCreditAsync`, antes del loop de cuotas) y el rescate a una cuenta (`RescueAsync`). `AmountMaterialized` lleva cuánto salió ya de la tarjeta; `Amount - AmountMaterialized` es el saldo a favor pendiente, que no se guarda en `Card` sino que se calcula.

`MaterializeAsync` reparte FIFO **solo entre cuotas que todavía no se pagaron** (se consultan los meses de `CardPayments`), topando cada una a `InstallmentAmount` menos lo ya etiquetado por materializaciones anteriores. El plan se arma entero y se valida antes de escribir: si el monto no entra en ninguna cuota, tira `BusinessRuleException` sin haber creado nada. Esa guarda es la que sostiene el invariante — **nunca se crea un ingreso que no tenga una cuota donde morir**.

**Cuánto saldo a favor se usó no se pide, se deduce** (ver D6/D6bis del plan). El banco lo aplica solo contra el total del resumen, así que: si el pago es mayor a $0 el crédito se agotó y se usó todo el disponible; si el pago es $0 el crédito cubrió cuotas + impuestos, y ahí "Gastos Tarjeta" —normalmente calculado— pasa a completarse a mano, porque es el único caso en que el pago deja de traer información. Siempre hay un solo número a cargar.

**Pase entre cuentas.** Abaratar una cuota es un canje: desaparece el ingreso y se achica el egreso por el mismo importe. Si el ingreso vive en otra cuenta (porque se rescató ahí), `EmitCrossAccountTransferAsync` emite un par `MovementType = "EX"` sin clase entre la cuenta que paga y la del ingreso. Sin eso las dos cuentas quedan descuadradas contra su extracto, aunque sumadas den bien. Mismo patrón que `TransactionService.RefundTransactionAsync`.

**Efecto conocido y aceptado:** mientras la tarjeta suelte crédito más rápido de lo que las cuotas lo absorben, queda un ingreso vivo que se consume y se vuelve a generar cada mes. Es aritmética, no un bug: lo vivo es siempre `materializado − aplicado`, y su techo es el descuento entero. Se evaluó rediseñarlo para eliminar ese churn (dejar quieto el sobrante en vez de preasignarlo a cuotas futuras) y **se decidió no hacerlo** — ver el plan.
