...
{
    /// <inheritdoc />
    public partial class GetStockStatsSplitFix : Migration
    {
        /// <inheritdoc />
...
BEGIN
    ...
    WITH TransactionsWithQuotes AS (
        SELECT
            t.AssetId,
            t.Amount,
            t.QuotePrice,
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN twq.LatestQuote > 0
                THEN (twq.Amount / twq.LatestQuote) * @LatestReferenceQuote
                ELSE 0
            END
        ) AS ActualValue
    FROM TransactionsWithQuotes twq
