...
namespace JazFinanzasApp.API.Business.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<(bool Succeeded, IEnumerable<string> Errors)> RegisterAsync(RegisterUserDTO dto)
        {
            ...
            // Ajuste Saldos Ingreso/Egreso e Inversiones/Ingreso Inversiones no representan un ingreso o
            // egreso real (son ajustes de saldo o el propio movimiento de invertir) — no cuentan en los
            // reportes de ingresos y egresos (D-3). Gastos Tarjeta y Reintegro sí cuentan.
            await _transactionClassRepository.AddAsync(new TransactionClass { Description = "Ajuste Saldos Ingreso", IncExp = "I", UserId = result.UserId, IsSystem = true, CountsAsIncomeExpense = false });
            await _transactionClassRepository.AddAsync(new TransactionClass { Description = "Ingreso Inversiones", IncExp = "I", UserId = result.UserId, IsSystem = true, CountsAsIncomeExpense = false });
            await _transactionClassRepository.AddAsync(new TransactionClass { Description = "Ajuste Saldos Egreso", IncExp = "E", UserId = result.UserId, IsSystem = true, CountsAsIncomeExpense = false });
            ...
            await _transactionClassRepository.AddAsync(new TransactionClass { Description = "Inversiones", IncExp = "E", UserId = result.UserId, IsSystem = true, CountsAsIncomeExpense = false });
